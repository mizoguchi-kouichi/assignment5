...
| 記号 | 意味 | 例 |
|:---:|:---:|:---:|
|？|クエリパラメータを付与します|パスパラメータの後ろ |
|＝ |名前と値を付与します |パラメータの名前と値の間 |
|＆|複数のクエリパラメータを付与する |クエリパラメータ同士の間 |
|＃|ページ内リンクを挿入する|クエリパラメータの後ろ|
...
# パス変数  
### ◎パス変数とは  
  -  パス変数とは、パソコンの環境変数という動作設定項目の一つになります。
  -  パス変数は、パスという実行ファイルまでの経路をパス変数に認識させる事により  
     コマンド入力の際に実行ファイル名のみで実行が出来るようになります。
